/** MODIFIED LINES 114-117 FOR SVCONNECT PORTAL TO FIX KNOWN BUG **/
			...
			var api = $(this).tooltip().onBeforeShow(function(e, pos) {				
				...
				*/
				...
				if (!isVisible(crop)) {
					...
					if (crop[2]) { $.extend(true, tipConf, conf.top);		tipConf.position[0] = 'top'; 		tip.addClass(cls[0]); }
					if (crop[3]) { $.extend(true, tipConf, conf.right);	tipConf.position[1] = 'right'; 	tip.addClass(cls[1]); }					
					if (crop[0]) { $.extend(true, tipConf, conf.bottom); 	tipConf.position[0] = 'bottom';	tip.addClass(cls[2]); } 
					if (crop[1]) { $.extend(true, tipConf, conf.left);		tipConf.position[1] = 'left'; 	tip.addClass(cls[3]); }					
